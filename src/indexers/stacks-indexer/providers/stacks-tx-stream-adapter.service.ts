...
export class StacksTxStreamAdapterService implements TxStreamAdapter {
  ...
  transformTx(tx: StacksTransaction): CommonTx {
    try {
      ...
      const notify = moment(new Date(tx.tx.burn_block_time)).utc() > moment().subtract(2, "hours").utc() ? true : false;
  ...
  subscribeToEvents(): Client {
    ...
    client.query("LISTEN new_block;", (err, res) => {
      this.logger.log("Listening to streamer block notifications");
    });
